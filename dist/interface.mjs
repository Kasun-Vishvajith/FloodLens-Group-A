...
async function checkArchiveDate(){
 ...
 try{
  const base=window.FLOODLENS_CONFIG?.apiBase;
  const response=await fetch(base?base+'/api/summary':'data/summary.json',{cache:'no-store',signal:AbortSignal.timeout(10000)});
  if(!response.ok)return;
  const latest=await response.json();
  if(latest.end>loadedEnd&&!document.getElementById('archive-update')){
