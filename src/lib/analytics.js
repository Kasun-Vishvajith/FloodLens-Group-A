...
export function episodes(rows,threshold=99,kind='flood'){
 ...
 let previous=null;
 for(const row of rows){if(previous&&Date.parse(row.date)-Date.parse(previous)!==86400000)current=null;previous=row.date;const p=kind==='heat'?row.tp:kind==='wind'?row.wp:kind==='rain'?row.rp:maximum([row.qp,row.rp]);
...
export function ordinal(value) {
 ...
 const n = Math.round(value), lastTwo = Math.abs(n) % 100;
 const suffix = lastTwo >= 11 && lastTwo <= 13 ? 'th' : ({1:'st',2:'nd',3:'rd'}[Math.abs(n)%10] || 'th');
 return `${n}${suffix}`;
}
export const logTick = (maximum, fraction) => Math.expm1(Math.log1p(maximum) * fraction);
